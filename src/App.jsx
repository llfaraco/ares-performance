...
function WorkoutModal(p){
  ...
  return<div style={{position:"fixed",inset:0,zIndex:200,display:"flex",flexDirection:"column",justifyContent:"flex-end"}}>
    ...
    <div className="pop" style={{background:C.bg,borderRadius:"20px 20px 0 0",position:"relative",zIndex:1,maxHeight:"90vh",display:"flex",flexDirection:"column",paddingBottom:SAB}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"14px 16px 40px"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}} className="au">
          {tab==="main"?items.map(function(ex,i){var inst=getExInstruction(ex.n),exLoad=loads[i]||{};return<Card key={i} style={{padding:0}}><div style={{display:"flex"}}><div style={{width:3,background:tc(session?session.type:""),flexShrink:0}}/><div style={{flex:1,padding:"12px 14px"}}><div style={{color:C.text,fontWeight:700,fontSize:14}}>{(i+1)+". "+ex.n}</div><div style={{display:"flex",gap:6,marginTop:5,flexWrap:"wrap"}}><span style={{background:C.redLight,padding:"2px 8px",borderRadius:4,fontSize:11,color:C.red,fontWeight:600}}>{ex.s}</span><span style={{background:C.blueBg,padding:"2px 8px",borderRadius:4,fontSize:11,color:C.blue,fontWeight:600}}>{ex.r}</span>{ex.rest&&<span style={{color:C.grayLight,fontSize:11}}>{ex.rest}</span>}</div>{!session.completed&&<div style={{marginTop:10,padding:"10px 12px",background:C.faint,borderRadius:8,border:"1px solid "+C.border}}><div style={{color:C.gray,fontSize:9,fontWeight:700,marginBottom:6}}>REGISTRAR CARGA</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>{[["kg","Kg","80"],["reps","Reps","8"],["rpe","RPE","7"]].map(function(arr){return<div key={arr[0]}><div style={{color:C.grayLight,fontSize:9,marginBottom:2}}>{arr[1]}</div><input type="number" value={exLoad[arr[0]]||""} placeholder={arr[2]} onChange={function(e){setLoad(i,arr[0],e.target.value);}} style={{width:"100%",background:C.white,border:"1px solid "+C.border,borderRadius:6,padding:"7px 8px",fontSize:13,outline:"none",minHeight:34}}/></div>;})}</div>{exLoad.kg&&exLoad.reps&&<div style={{marginTop:6,color:C.green,fontSize:10,fontWeight:600}}>{"Volume: "+(parseInt(exLoad.kg)*parseInt(exLoad.reps))+" kg"}</div>}</div>}{inst&&<div style={{marginTop:8,padding:"8px 10px",background:C.blueBg,borderRadius:6}}><div style={{color:C.blue,fontSize:9,fontWeight:700,marginBottom:3}}>COMO EXECUTAR</div><div style={{color:C.text,fontSize:11,lineHeight:1.5}}>{inst}</div></div>}{ex.notes&&<div style={{marginTop:6,background:C.amberBg,borderRadius:6,padding:"8px 10px",color:C.amber,fontSize:12}}>{ex.notes}</div>}{ex.alt&&<div style={{marginTop:4,color:C.grayLight,fontSize:11}}>{"Alt: "+ex.alt}</div>}</div></div></Card>;}):items.map(function(w,i){return<Card key={i} style={{padding:"12px 14px"}}><div style={{color:C.text,fontWeight:600,fontSize:13}}>{w.n}</div><div style={{color:tab==="warmup"?C.amber:tab==="cardio"?C.green:C.blue,fontSize:12,marginTop:2}}>{w.d}</div>{w.notes&&<div style={{color:C.grayLight,fontSize:11,marginTop:4}}>{w.notes}</div>}</Card>;})}
        </div>
      </div>
    </div>
  </div>;
...
function LoginScreen(){
  ...
  if(mode==="welcome")return<div style={{minHeight:"100vh",background:C.dark,display:"flex",flexDirection:"column"}}>
    ...
    <div style={{background:C.white,borderRadius:"24px 24px 0 0",padding:"32px 24px calc(40px + "+SAB+")",animation:"slideUp .4s ease"}}>
...
function DashboardTab(p){
  ...
  var weekPct=wt2>0?Math.round((wd/wt2)*100):0,cyclePct=plan?Math.round(((plan.currentWeek-1)/12)*100):0;
  var weekDays=["S","T","Q","Q","S","S","D"],today=new Date().getDay(),todayIdx=today===0?6:today-1;
  var totalSessions=sessions.length,sm=getSportMetrics(activity?activity.sport:"");
  ...
  return<div className="au" style={{display:"flex",flexDirection:"column",gap:12,padding:"0 0 20px"}}>
    ...
    {dashView==="esporte"&&<div style={{display:"flex",flexDirection:"column",gap:10}}>
      {sm.metrics.length>0&&<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><span style={{color:C.gray,fontSize:10,fontWeight:600}}>{"METRICAS "+(sp?sp.name.toUpperCase():"")}</span><Btn sm v="ghost" onClick={function(){setShowMetricEditor(!showMetricEditor);}}>{showMetricEditor?"Fechar":"Editar"}</Btn></div><div style={{display:"grid",gridTemplateColumns:"repeat("+Math.min(sm.metrics.length,3)+",1fr)",gap:8}}>{sm.metrics.slice(0,3).map(function(m){var val=metricValues[m.id],hasVal=val!==undefined&&val!=="";return<Card key={m.id} style={{padding:"12px 10px",textAlign:"center"}}><div style={{fontSize:16,marginBottom:4}}>{m.icon}</div><div style={{color:C.grayLight,fontSize:9,fontWeight:600,marginBottom:4}}>{m.label.toUpperCase()}</div><div style={{color:hasVal?C.text:C.grayLight,fontSize:20,fontWeight:800,fontFamily:"'Bebas Neue',sans-serif",lineHeight:1}}>{hasVal?val:"--"}</div><div style={{color:hasVal?C.green:C.grayLight,fontSize:9,marginTop:2}}>{hasVal?m.unit:"toque editar"}</div></Card>;})}
      </div>{showMetricEditor&&<Card style={{padding:16,marginTop:8}}><div style={{color:C.gray,fontSize:11,fontWeight:600,marginBottom:10}}>Inserir metricas</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>{sm.metrics.map(function(m){return<div key={m.id}><div style={{color:C.grayLight,fontSize:10,fontWeight:600}}>{m.icon+" "+m.label}</div><input type="number" step="0.01" value={metricValues[m.id]||""} placeholder={"Ex: "+(m.lower?"4.5":"100")} onChange={function(e){setMetricValues(function(prev){var n=Object.assign({},prev);n[m.id]=e.target.value;return n;});}} style={{background:C.faint,border:"1px solid "+C.border,borderRadius:8,color:C.text,padding:"9px 10px",fontSize:13,outline:"none",minHeight:40,width:"100%",marginTop:4}}/></div>;})}
      </div><Btn onClick={function(){setShowMetricEditor(false);}} sm full style={{marginTop:10}}>Salvar</Btn></Card>}</div>}
      ...
      {cw&&<div><div style={{color:C.gray,fontSize:10,fontWeight:600,marginBottom:6}}>{"SESSOES SEMANA "+cw.week}</div><div style={{display:"flex",flexDirection:"column",gap:5}}>{cw.sessions.map(function(s,i){return<div key={s.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:s.completed?C.greenBg:C.white,borderRadius:10,border:"1px solid "+(s.completed?C.green+"22":C.border),minHeight:48}}><div style={{width:24,height:24,borderRadius:"50%",background:s.completed?C.green:C.faint,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,flexShrink:0,color:s.completed?"#fff":C.grayLight,fontWeight:700}}>{s.completed?"✓":i+1}</div><div style={{flex:1,minWidth:0}}><div style={{color:s.completed?C.green:C.text,fontWeight:600,fontSize:13}}>{s.name}</div><div style={{color:C.grayLight,fontSize:10,marginTop:1}}>{s.type+" - "+s.duration+"min"}</div></div><div style={{width:5,height:5,borderRadius:"50%",background:tc(s.type)}}/></div>;})}
      </div></div>}
    </div>}
    {dashView==="semana"&&<div style={{display:"flex",flexDirection:"column",gap:10}}><Card style={{padding:"24px 18px"}}><div style={{display:"flex",alignItems:"center",gap:20,flexWrap:"wrap",justifyContent:"center"}}><div style={{position:"relative",width:120,height:120,flexShrink:0}}><svg width={120} height={120} style={{position:"absolute",transform:"rotate(-90deg)"}}><circle cx={60} cy={60} r={50} fill="none" stroke={C.faint} strokeWidth={10}/><circle cx={60} cy={60} r={50} fill="none" stroke={C.red} strokeWidth={10} strokeDasharray={(2*Math.PI*50*Math.min(weekPct,100)/100)+" "+(2*Math.PI*50*(1-Math.min(weekPct,100)/100))} strokeLinecap="round"/></svg><div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:32,color:C.text,lineHeight:1}}>{wd+"/"+wt2}</div><div style={{fontSize:9,color:C.grayLight,marginTop:1}}>SESSOES</div></div></div><div style={{flex:1,minWidth:140}}><div style={{display:"flex",gap:5,marginBottom:12}}>{weekDays.map(function(day,i){var isDone=i<wd,isToday=i===todayIdx;return<div key={i} style={{flex:1,textAlign:"center"}}><div style={{width:28,height:28,borderRadius:"50%",margin:"0 auto 3px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,background:isDone?C.red:isToday?C.redLight:C.faint,color:isDone?"#fff":isToday?C.red:C.grayLight,border:isToday&&!isDone?"1.5px solid "+C.red:"1.5px solid transparent"}}>{isDone?"✓":i+1}</div><div style={{fontSize:8,color:isToday?C.red:C.grayLight}}>{day}</div></div>;})}
      </div><div style={{display:"flex",alignItems:"center",gap:6,padding:"6px 10px",background:cw?(cw.phaseColor+"0A"):C.faint,borderRadius:6,border:"1px solid "+(cw?(cw.phaseColor+"22"):C.border)}}><div style={{width:6,height:6,borderRadius:"50%",background:cw?cw.phaseColor:C.gray}}/><span style={{fontSize:12,fontWeight:600,color:C.text}}>{cw?cw.phase:""}</span><span style={{color:C.grayLight,fontSize:10,marginLeft:"auto"}}>{cw?cw.intensity:""}</span></div></div></div></Card>
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}><Card style={{padding:"12px 10px",textAlign:"center"}}><div style={{color:C.grayLight,fontSize:9,fontWeight:600,marginBottom:4}}>ADERENCIA</div><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:C.text}}>{weekPct+"%"}</div></Card><Card style={{padding:"12px 10px",textAlign:"center"}}><div style={{color:C.grayLight,fontSize:9,fontWeight:600,marginBottom:4}}>STREAK</div><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:C.text}}>{streak>0?streak:"--"}</div></Card><Card style={{padding:"12px 10px",textAlign:"center"}}><div style={{color:C.grayLight,fontSize:9,fontWeight:600,marginBottom:4}}>TOTAL</div><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:C.text}}>{totalSessions}</div></Card></div></div>}
    {dashView==="treino"&&<div style={{display:"flex",flexDirection:"column",gap:10}}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}><Card style={{padding:"12px 10px",textAlign:"center"}}><div style={{color:C.grayLight,fontSize:9,fontWeight:600,marginBottom:4}}>VOLUME SEM.</div><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:C.text}}>--</div></Card><Card style={{padding:"12px 10px",textAlign:"center"}}><div style={{color:C.grayLight,fontSize:9,fontWeight:600,marginBottom:4}}>RPE MEDIO</div><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:C.text}}>--</div></Card><Card style={{padding:"12px 10px",textAlign:"center"}}><div style={{color:C.grayLight,fontSize:9,fontWeight:600,marginBottom:4}}>SESSOES</div><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:C.text}}>{totalSessions}</div></Card></div><Card style={{padding:"18px 16px"}}><SL mb={10}>Ultimas sessoes</SL>{sessions.length>0?sessions.slice(0,5).map(function(s,i){return<div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:i<4?"1px solid "+C.faint:"none"}}><div style={{width:6,height:6,borderRadius:"50%",background:tc(s.type)}}/><div style={{flex:1}}><div style={{fontWeight:600,fontSize:12}}>{s.type}</div><div style={{color:C.grayLight,fontSize:10}}>{(s.duration||0)+"min"}</div></div><div style={{color:C.grayLight,fontSize:10}}>{s.date||""}</div></div>;}): <div style={{padding:"16px 0",textAlign:"center",color:C.grayLight,fontSize:12}}>Nenhuma sessao</div>}</Card></div>}
    {dashView==="progresso"&&<div style={{display:"flex",flexDirection:"column",gap:10}}><Card style={{padding:"18px 16px"}}><SL mb={10}>Ciclo 12 semanas</SL><div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{color:C.grayLight,fontSize:9,fontWeight:600}}>PROGRESSO</span><span style={{fontWeight:700,fontSize:12,color:C.text}}>{cyclePct+"%"}</span></div><div style={{display:"flex",gap:2}}>{(plan?plan.weeks:[]).map(function(w){var done=w.sessions.filter(function(s){return s.completed;}).length,tot=w.sessions.length,allD=done===tot&&tot>0,cur=w.week===(plan?plan.currentWeek:1);return<div key={w.week} style={{flex:1,height:8,borderRadius:99,background:allD?C.green:cur?w.phaseColor:C.faint}}/>;})}</div></Card><Card style={{padding:"18px 16px"}}><SL mb={10}>Experiencia</SL><XPBar xp={activity?activity.xp||0:0}/></Card></div>}
    {dashView==="corpo"&&<div style={{display:"flex",flexDirection:"column",gap:10}}><Card style={{padding:"18px 16px"}}><SL mb={10}>Medidas corporais</SL><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>{[{id:"peso",label:"Peso (kg)",icon:"⚖️"},{id:"gordura",label:"Gordura (%)",icon:"📐"},{id:"cintura",label:"Cintura (cm)",icon:"📏"},{id:"braco",label:"Braco (cm)",icon:"💪"}].map(function(m){return<div key={m.id}><div style={{color:C.grayLight,fontSize:10,fontWeight:600,marginBottom:3}}>{m.icon+" "+m.label}</div><input type="number" step="0.1" placeholder="--" style={{width:"100%",background:C.faint,border:"1px solid "+C.border,borderRadius:8,padding:"9px 10px",fontSize:13,outline:"none",minHeight:40,color:C.text}}/></div>;})}
    </div><Btn sm full style={{marginTop:12}}>Salvar medidas</Btn></Card></div>}
  </div>;
}
...
function AITab(p){
  ...
  return<div className="au" style={{display:"flex",flexDirection:"column",gap:12,padding:"0 0 20px"}}><div><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:28,color:C.text,letterSpacing:1}}>INTELIGENCIA</div><div style={{color:C.grayLight,fontSize:12,marginTop:2}}>Analise com IA</div></div><Card style={{padding:20}}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:48,height:48,background:C.redLight,borderRadius:12,display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0}}>🧠</div><div style={{flex:1}}><div style={{fontWeight:700,fontSize:14}}>ARES INTELLIGENCE</div><div style={{color:C.grayLight,fontSize:11,marginTop:1}}>{credits>0?credits+" creditos":"Sem creditos"}</div></div><Btn onClick={gen} disabled={ld||credits<=0} sm>{ld?"...":"Gerar"}</Btn></div></Card>{reports.map(function(r){return<Card key={r.id} red style={{padding:20}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{color:C.gray,fontSize:11,fontWeight:600}}>{r.date}</span>{r.done&&<Tag color={C.green}>OK</Tag>}</div><pre ref={ref} style={{color:C.text,fontSize:12,lineHeight:1.8,fontFamily:"monospace",whiteSpace:"pre-wrap",margin:0,maxHeight:300,overflowY:"auto"}}>{r.typed}{!r.done&&<span style={{color:C.red,animation:"blink .6s infinite"}}>|</span>}</pre></Card>;})}
  </div>;
}

/* PROFILE com trocar esporte + compartilhar */
function ProfileTab(p){
  ...
  var[saved,sSaved]=useState(false),[billing,sBilling]=useState("mensal");
  ...
  /* PLANOS */
  if(section==="plans")return<div className="au" style={{display:"flex",flexDirection:"column",gap:12,padding:"0 0 20px"}}><div style={{display:"flex",alignItems:"center",gap:8}}><button onClick={function(){setSection("main");}} style={{width:32,height:32,borderRadius:8,background:C.faint,border:"1px solid "+C.border,cursor:"pointer",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center"}}>{"<"}</button><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:24,color:C.text}}>PLANOS</div></div><div style={{display:"flex",justifyContent:"center"}}><div style={{display:"inline-flex",background:C.faint,borderRadius:10,padding:2,gap:2}}>{["mensal","anual"].map(function(b){return<button key={b} onClick={function(){sBilling(b);}} style={{padding:"8px 18px",border:"none",cursor:"pointer",background:billing===b?C.white:"transparent",color:billing===b?C.red:C.gray,fontWeight:700,fontSize:11,textTransform:"uppercase",borderRadius:8,minHeight:36,display:"flex",alignItems:"center",gap:4}}>{b}{b==="anual"&&<span style={{background:C.greenBg,color:C.green,fontSize:8,padding:"1px 5px",borderRadius:99}}>-20%</span>}</button>;})}
  </div></div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:10}}>{plans.map(function(pl){var isCur=(user?user.plan:"free")===pl.id;return<Card key={pl.id} red={isCur} style={{padding:20,display:"flex",flexDirection:"column"}}><div style={{color:pl.color,fontWeight:700,fontSize:12,letterSpacing:1,marginBottom:8}}>{pl.name}</div><div style={{display:"flex",alignItems:"flex-end",gap:3,marginBottom:14}}><span style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:28,color:C.text,lineHeight:1}}>{pl.price[billing]}</span><span style={{color:C.grayLight,fontSize:10,marginBottom:2}}>/mes</span></div><div style={{height:1,background:C.border,marginBottom:12}}/><div style={{display:"flex",flexDirection:"column",gap:5,flex:1}}>{pl.features.map(function(f){return<div key={f} style={{display:"flex",gap:6}}><span style={{color:C.green,fontSize:11}}>✓</span><span style={{fontSize:12,color:C.text}}>{f}</span></div>;})}</div><div style={{marginTop:14}}><Btn v={isCur?"ghost":"primary"} full sm>{isCur?"Atual":"Upgrade"}</Btn></div></Card>;})}
  </div></div>;

  /* COMPARTILHAR */
  if(section==="share")return<div className="au" style={{display:"flex",flexDirection:"column",gap:12,padding:"0 0 20px"}}>
    <div style={{display:"flex",alignItems:"center",gap:8}}><button onClick={function(){setSection("main");}} style={{width:32,height:32,borderRadius:8,background:C.faint,border:"1px solid "+C.border,cursor:"pointer",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center"}}>{"<"}</button><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:24,color:C.text}}>COMPARTILHAR</div></div>
    <Card style={{padding:20}}>
      <div style={{borderRadius:10,overflow:"hidden",marginBottom:14}}>
        <svg viewBox="0 0 520 240" style={{width:"100%",display:"block"}}><rect width="520" height="240" fill="#0A0A0A"/><rect width="520" height="3" fill={C.red}/><text x="24" y="30" fill={C.red} fontSize="10" fontFamily="Inter,sans-serif" fontWeight="700" letterSpacing="3">ARES PERFORMANCE</text><text x="496" y="30" fill="white" fontSize="12" fontFamily="Inter,sans-serif" fontWeight="700" textAnchor="end">{(user?user.name:"ATLETA").toUpperCase()}</text><text x="24" y="80" fill="white" fontSize="52" fontFamily="Inter,sans-serif" fontWeight="900">{activity?activity.sessions?activity.sessions.length:0:0}</text><text x="24" y="100" fill={C.red} fontSize="9" fontFamily="Inter,sans-serif" fontWeight="600" letterSpacing="2">SESSOES</text><text x="24" y="140" fill="#ffffff66" fontSize="11" fontFamily="Inter,sans-serif">{sp?(sp.icon+" "+sp.name):""}</text><text x="24" y="230" fill="#ffffff22" fontSize="7" fontFamily="Inter,sans-serif">ares.performance.app</text></svg>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6}}>
        {[{n:"WhatsApp",c:"#25D366",i:"💬"},{n:"Twitter",c:"#1DA1F2",i:"🐦"},{n:"Instagram",c:"#E1306C",i:"📸"},{n:"Telegram",c:"#0088CC",i:"📨"},{n:"Strava",c:"#FC4C02",i:"🏃"},{n:"LinkedIn",c:"#0077B5",i:"💼"}].map(function(s){return<button key={s.n} onClick={function(){var txt=encodeURIComponent("Treino concluido! "+(sp?sp.icon+" "+sp.name:"")+"\n#ARESPerformance");var urls={WhatsApp:"https://wa.me/?text="+txt,Twitter:"https://twitter.com/intent/tweet?text="+txt,Instagram:"https://instagram.com",Telegram:"https://t.me/share/url?text="+txt,Strava:"https://strava.com",LinkedIn:"https://linkedin.com"};window.open(urls[s.n]||"#","_blank");}} style={{padding:"12px 6px",background:s.c+"0A",border:"1px solid "+s.c+"22",borderRadius:10,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:4,minHeight:56}}><span style={{fontSize:18}}>{s.i}</span><span style={{fontSize:9,fontWeight:600,color:C.text}}>{s.n}</span></button>;})}
      </div>
    </Card>
  </div>;
  ...
  if(section==="sport")return<div className="au" style={{display:"flex",flexDirection:"column",gap:12,padding:"0 0 20px"}}>
    <div style={{display:"flex",alignItems:"center",gap:8}}><button onClick={function(){setSection("main");}} style={{width:32,height:32,borderRadius:8,background:C.faint,border:"1px solid "+C.border,cursor:"pointer",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center"}}>{"<"}</button><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:24,color:C.text}}>TROCAR ESPORTE</div></div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(130px,1fr))",gap:8}}>
      {SPORTS.map(function(s){var isCur=activity&&activity.sport===s.id;return<Card key={s.id} hover red={isCur} onClick={function(){if(!isCur&&p.onChangeSport)p.onChangeSport(s.id);setSection("main");}} style={{padding:"14px 12px",cursor:"pointer"}}><div style={{fontSize:22,marginBottom:6}}>{s.icon}</div><div style={{color:C.text,fontWeight:700,fontSize:13}}>{s.name}</div><div style={{color:C.grayLight,fontSize:10,marginTop:2}}>{s.cat}</div>{isCur&&<div style={{position:"absolute",top:6,right:6}}><Tag color={C.green}>ATUAL</Tag></div>}</Card>;})}
    </div>
  </div>;
  ...
  return<div className="au" style={{display:"flex",flexDirection:"column",gap:12,padding:"0 0 20px"}}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:6}}>
      {[{id:"sport",label:"Trocar esporte",icon:"🏅",desc:"Mude seu esporte principal"},{id:"plans",label:"Planos ARES",icon:"⭐",desc:"Upgrade e assinatura"},{id:"share",label:"Compartilhar",icon:"📤",desc:"Cards para redes sociais"}].map(function(m){return<Card key={m.id} hover onClick={function(){setSection(m.id);}} style={{padding:"14px 16px",cursor:"pointer"}}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{width:36,height:36,borderRadius:8,background:C.faint,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>{m.icon}</div><div style={{flex:1}}><div style={{fontWeight:700,fontSize:13}}>{m.label}</div><div style={{color:C.grayLight,fontSize:11,marginTop:1}}>{m.desc}</div></div><span style={{color:C.grayLight}}>{">"}</span></div></Card>;})}
    </div>
...
function MainApp(p){
  ...
  function changeSport(newSport){
    setActivities(function(prev){
      var updated=prev.map(function(a,i){
        if(i!==activeIdx)return a;
        var newActivity=Object.assign({},a,{sport:newSport,plan:generatePlan(Object.assign({},a,{sport:newSport}))});
        if(p.onSaveActivity)p.onSaveActivity(newActivity);
        return newActivity;
      });
      return updated;
    });
  ...
  return<div style={{background:C.bg,minHeight:"100vh"}}>
    ...
    <div style={{background:C.white,borderBottom:"1px solid "+C.border,padding:"0 16px",paddingTop:SAT,display:"flex",alignItems:"center",gap:8,position:"sticky",top:0,zIndex:100,boxShadow:sh.sm,minHeight:"calc(52px + "+SAT+")"}}>
      <div style={{flex:1,paddingTop:4}}><AresLogo size={28}/></div>
      {sp&&<div style={{padding:"5px 12px",background:C.faint,borderRadius:20,fontSize:12,fontWeight:600,color:C.text,marginTop:4}}>{sp.icon+" "+sp.name}</div>}
      <div onClick={function(){sTab("me");}} style={{width:32,height:32,background:C.red,borderRadius:8,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:800,fontSize:11,cursor:"pointer",marginTop:4}}>{(user?user.name:"A").charAt(0).toUpperCase()}</div>
    </div>

    {/* BOTTOM NAV com safe area */}
    <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:100}}>
      <div style={{background:C.white,borderTop:"1px solid "+C.border,display:"flex",alignItems:"flex-end",justifyContent:"space-around",paddingBottom:SAB,minHeight:60}}>
        {TABS.map(function(t){
          if(t.fab)return<div key="fab" style={{marginTop:-20,marginBottom:4}}><button onClick={function(){sTab("reg");}} className={tab!=="reg"?"fab-pulse":""} style={{width:52,height:52,borderRadius:14,background:C.red,border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 16px "+C.red+"44"}}><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round"><path d="M12 5v14M5 12h14"/></svg></button></div>;
          return<button key={t.id} onClick={function(){sTab(t.id);}} style={{flex:1,padding:"8px 4px 4px",border:"none",cursor:"pointer",background:"transparent",display:"flex",flexDirection:"column",alignItems:"center",gap:2,maxWidth:72}}><TI d={t.d} active={tab===t.id}/><div style={{fontSize:9,fontWeight:700,color:tab===t.id?C.red:C.grayLight}}>{t.label}</div></button>;
        })}
      </div>
    </div>

    <div style={{maxWidth:800,margin:"0 auto",padding:"16px 14px calc(80px + "+SAB+")"}}>
      <div key={tab} className="au">
        ...
        {tab==="me"&&<ProfileTab user={Object.assign({},user,{plan:userPlan})} activity={ca} onLogout={p.onLogout} onSaveProfile={p.onSaveProfile} onChangeSport={changeSport}/>}
      </div>
      {activities.length===0&&tab!=="me"&&<Card style={{padding:28,textAlign:"center",marginTop:16}}><div style={{fontSize:36,marginBottom:10}}>🏅</div><div style={{fontFamily:"'Bebas Neue',sans-serif",fontSize:22,color:C.text,marginBottom:8}}>COMECE AGORA</div><div style={{color:C.gray,fontSize:13,marginBottom:16}}>Adicione seu esporte.</div><Btn onClick={function(){sShowOb(true);}}>Adicionar esporte</Btn></Card>}
    </div>
  </div>;
}
